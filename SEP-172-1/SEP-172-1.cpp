// SEP-172-1.cpp : Defines the entry point for the console application.
//
...
/*
Задание:
1. Доделать класс MyVector.
2. Создайте шаблонный класс квадратной матрицы matrix. Необходимо реализовать динамическое выделение памяти,
очистку памяти, заполнение матрицы с клавиатуры, заполнение случайными значениями, отображение
матрицы, арифметические операции с помощью перегруженных операторов (+, –, *, /), поиск
...
MyVector<T>& MyVector<T>::operator+=(MyVector<T>& obj)
{
	...
	for (int j = size; j < size + obj.size; j++)
	{
